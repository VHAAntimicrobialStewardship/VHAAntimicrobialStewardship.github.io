...
def audit_station(station_dir: str, omjson_filename: str, odjson_filename: str, html_filename: str, combined_only: bool) -> tuple[list[str], list[str]]:
    ...
    od_path = ROOT / "stations" / station_dir / odjson_filename
    html_path = ROOT / "stations" / station_dir / html_filename
    ...
    if combined_only:
        ...
        for check in html_forbidden:
            if check in html:
                issues.append(f"HTML should not contain legacy tab button (combined-only station): {check}")
    else:
        html_checks = [
